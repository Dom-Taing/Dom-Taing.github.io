...
export default function Hero() {
  ...
  return (
    <section
      ref={ref}
      id="hero"
      className="relative h-[65vh] min-h-[480px] max-mobile:min-h-[520px] bg-hero-bg flex items-end overflow-hidden"
    >
      {/* Dot grid overlay */}
      <div
      ...
      {/* Concentric circles + accent line — hidden on mobile */}
      <div className="absolute inset-0 pointer-events-none max-mobile:hidden">
      ...
      {/* Accent line on mobile (no circles) */}
      <div
        className="absolute left-0 bottom-0 w-full h-[3px] pointer-events-none hidden max-mobile:block"
        style={{ background: 'linear-gradient(to right, var(--hanada) 0%, var(--tokiwa) 40%, transparent 70%)' }}
      />
      ...
      <motion.div
        style={{ y, opacity }}
        className="relative z-10 w-full px-14 pb-[52px] max-lg:px-8 max-lg:pb-12 max-mobile:px-5 max-mobile:pb-11"
      >
        {/* Eyebrow */}
        ...
        <div className="overflow-hidden">
          <motion.div
            className="flex items-center gap-6 max-mobile:flex-col max-mobile:items-start max-mobile:gap-4"
            {...dropIn(0.65)}
          >
            <a
              href="#experience"
      ...
      <div className="absolute right-14 max-lg:right-8 bottom-[52px] z-10 flex flex-col items-center gap-2.5 text-white/25 text-[10px] tracking-[0.18em] uppercase [writing-mode:vertical-rl] max-mobile:hidden">
        <div
          className="w-px h-12"
          style={{
