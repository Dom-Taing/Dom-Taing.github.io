...
export default function TechStack({ stack }: { stack: Array<{ layer: string; items: string[] }> }) {
  return (
    <section className="px-14 py-[80px] bg-bg-warm border-t border-[rgba(27,85,166,0.08)]">
          ...
          <div key={layer.layer} className="bg-bg p-8 relative overflow-hidden">
            ...
            <div className="flex flex-wrap gap-2">
              {layer.items.map((item) => (
                <span key={item} className="text-[12px] px-2.5 py-1 bg-[rgba(27,85,166,0.08)] text-hanada rounded-sm font-medium">
                  {item}
                </span>
              ))}
            </div>
