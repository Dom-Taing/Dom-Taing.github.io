...
function ProjectCard({
  ...
  return (
    <motion.div
      ...
      className={`cursor-pointer [transform-style:preserve-3d] ${wide ? 'mobile:col-span-2' : ''}`}
    >
      <div
        ref={tiltRef}
        className={`relative bg-bg hover:bg-white transition-colors p-11 max-lg:p-8 max-mobile:px-6 max-mobile:py-7 overflow-hidden [transform-style:preserve-3d] will-change-transform ${
          wide ? 'grid grid-cols-1 mobile:grid-cols-2 gap-6 mobile:gap-12 items-center' : ''
        }`}
      >
        {/* Left accent bar */}
        ...
        {wide && (
          <div className="aspect-[16/10] overflow-hidden max-mobile:hidden">
...
export default function FeaturedProjects() {
  ...
  return (
    <section id="projects" className="relative bg-bg-warm px-14 max-lg:px-8 max-mobile:px-5 py-[100px] max-lg:py-20 max-mobile:py-16 border-t border-[rgba(29,92,58,0.15)] overflow-hidden">
      ...
      <div className="flex items-end justify-between mb-14 max-mobile:flex-col max-mobile:items-start max-mobile:gap-3">
        <div>
        ...
        <Link
          href="/projects"
          className="text-[12px] font-semibold tracking-[0.12em] uppercase text-tokiwa border-b border-current pb-0.5 mb-1.5 hover:text-yamabuki hover:border-yamabuki transition-colors whitespace-nowrap max-mobile:mb-0"
        >
          All Projects →
        </Link>
      </div>
